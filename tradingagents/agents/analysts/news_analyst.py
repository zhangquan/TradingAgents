...
def create_news_analyst(llm, toolkit, polygon_toolkit, config=None):
    def news_analyst_node(state):
        ...
        logger.info(f"News analyst language config: {report_language}")
        ...
        # Log language-specific configuration
        if language_sources:
            logger.info(f"Using language-specific news sources for {normalized_language}: {language_sources[:3]}")
        ...
        if language_sources and normalized_language != "en-US":
            ...
            logger.info(f"Added language-specific news sources context for {normalized_language}")
        
        # Add language-specific analysis instructions
        if normalized_language != "en-US":
            base_message += f" Focus on news and analysis relevant to {normalized_language} markets and economic conditions."
        ...
        if len(result.tool_calls) == 0:
            ...
            if auto_detect and report:
                logger.info("Auto-detection enabled, analyzing report content for language detection")
                detected_language = detect_language_from_text(report)
                logger.info(f"Detected language: {detected_language} (original: {normalized_language})")
                
                if detected_language != normalized_language:
                    logger.info(f"Language mismatch detected, re-generating report in {detected_language}")
                    ...
                    updated_base_message = "You are a news researcher tasked with analyzing recent news and trends over the past week. Please write a comprehensive report of the current state of the world that is relevant for trading and macroeconomics. Look at news from EODHD, and finnhub to be comprehensive. Do not simply state the trends are mixed, provide detailed and finegrained analysis and insights that may help traders make decisions."
                    ...
                    if detected_sources and detected_normalized != "en-US":
                        ...
                        logger.info(f"Added detected language news sources: {detected_sources[:3]}")
                    
                    # Add language-specific analysis instructions
                    if detected_normalized != "en-US":
                        updated_base_message += f" Focus on news and analysis relevant to {detected_normalized} markets and economic conditions."
                    ...
                    if len(updated_result.tool_calls) == 0:
                        ...
                        logger.info(f"Successfully re-generated report in detected language: {detected_normalized}")
                else:
                    logger.info("Detected language matches configured language, no re-generation needed")

        # Prepare return data with language metadata
        return_data = {
            "messages": [result],
            "news_report": report,
        }
        
        # Add language metadata for debugging and tracking
        if auto_detect:
            return_data["language_metadata"] = {
                "configured_language": report_language,
                "normalized_language": normalized_language,
                "auto_detection_enabled": True,
                "detected_language": detected_language if 'detected_language' in locals() else normalized_language,
                "final_language": detected_normalized if 'detected_normalized' in locals() else normalized_language
            }
        else:
            return_data["language_metadata"] = {
                "configured_language": report_language,
        ...
        logger.info(f"News analyst completed with language: {return_data['language_metadata']['final_language']}")
